...
const RISK_BG = {
  Low: 'var(--success-light)', Medium: 'var(--warning-light)',
  High: 'var(--danger-light)', Critical: 'var(--critical-light)'
}
const RISK_COLOR = {
...
function Section({ title, children }) {
  return (
    <section style={{ marginBottom: 32 }}>
      <h2 style={{ fontSize: 18, fontWeight: 700, marginBottom: 16, paddingBottom: 8,
        borderBottom: '2px solid var(--border)' }}>{title}</h2>
      {children}
    </section>
  )
}
...
export default function Results() {
  ...
  const riskLabel = { Low: r.riskLow, Medium: r.riskMedium, High: r.riskHigh, Critical: r.riskCritical }
    ...
    <>
      ...
      <main style={{ maxWidth: 860, margin: '0 auto', padding: '32px 24px' }}>
        ...
        <button onClick={() => navigate('/')} style={{
          ...
          padding: '8px 16px', fontSize: 14, color: 'var(--text-secondary)', marginBottom: 24,
          cursor: 'pointer'
        }}>{r.backBtn}</button>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
            <div>
              <div style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 4 }}>{fileName}</div>
              <div style={{ fontSize: 20, fontWeight: 700 }}>{result.document_type}</div>
              {result.ai_provider && (
          ...
          {result.summary && (
            <p style={{ marginTop: 16, color: 'var(--text-secondary)', lineHeight: 1.7 }}>{result.summary}</p>
          )}
        </div>
        ...
        {result.good_clauses?.length > 0 && (
          <Section title="✅ Good Clauses">
            {result.good_clauses.map((c, i) => (
              <div key={i} style={{
                background: 'var(--success-light)', borderLeft: '3px solid var(--success)',
                borderRadius: 'var(--radius)', padding: '10px 14px', marginBottom: 10,
              ...
              <div key={i} style={{
                ...
                {c.content && (
                  <div style={{ fontStyle: 'italic', color: 'var(--text-muted)', fontSize: 13,
                    background: 'var(--bg-subtle)', padding: '8px 12px', borderRadius: 6, marginBottom: 10 }}>
                    "{c.content}"
                  </div>
                )}
                <p style={{ fontSize: 14, color: 'var(--text-secondary)', marginBottom: 8 }}>{c.plain_english}</p>
                {c.indian_law_reference && (
                  <div style={{ fontSize: 13, color: 'var(--accent)', marginBottom: 6 }}>⚖️ {c.indian_law_reference}</div>
                )}
                {c.recommendation && (
                  <div style={{ fontSize: 13, background: 'var(--accent-light)', color: 'var(--accent)',
                    padding: '8px 12px', borderRadius: 6 }}>💡 {c.recommendation}</div>
                )}
              </div>
            ))}
          </Section>
        ...
        {result.future_implications && (
          <Section title={r.futureImplications}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16 }}>
              {[
                { label: '1 Month', value: result.future_implications.one_month },
                { label: '1 Year', value: result.future_implications.one_year },
                { label: 'Long Term', value: result.future_implications.long_term },
              ].map(item => item.value && (
          ...
          <Section title={r.lawCrossRef}>
            {result.applicable_laws.map((law, i) => (
              <div key={i} style={{
                ...
                <div>
                  <div style={{ fontWeight: 600, fontSize: 14 }}>{law.name}</div>
                  <div style={{ color: 'var(--text-secondary)', fontSize: 13, marginTop: 4 }}>{law.relevance}</div>
                </div>
              </div>
            ))}
          </Section>
          ...
          <Section title={r.negotiationTips}>
            {result.negotiation_tips.map((tip, i) => (
              <div key={i} style={{
                ...
                borderBottom: '1px solid var(--border)', fontSize: 14, color: 'var(--text-secondary)'
              }}>
        ...
        {/* Disclaimer */}
        <div style={{
          marginTop: 40, padding: 16, background: 'var(--bg-subtle)', border: '1px solid var(--border)',
          borderRadius: 'var(--radius)', fontSize: 13, color: 'var(--text-muted)', textAlign: 'center'
        }}>{r.disclaimer}</div>
