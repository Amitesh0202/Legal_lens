...
### Must Have
- [x] File upload supporting PDF, DOCX, TXT (up to 10MB)
- [x] AI-powered document analysis using Gemini 2.5 Flash
- [x] Plain-English summary of the document
- [x] Risk score (0–100) with label (Low / Medium / High / Critical)
- [x] Hidden clause detection with severity ratings
- [x] Indian law cross-reference (73 laws)
- [x] Future implications by timeframe
- [x] Negotiation tips
- [x] Laws reference page (searchable, filterable by category)
...
## Acceptance Criteria
- [x] User can upload a PDF and receive a complete analysis in under 30 seconds
- [x] Analysis correctly identifies at least 3 risky clauses in a standard employment contract
- [x] Risk score is calibrated: a standard NDA scores Low, a predatory loan scores Critical
- [x] All 73 laws appear in the Laws page with accurate explanations
- [x] App works correctly on mobile screens (375px+)
- [x] Error states are handled gracefully (unsupported file, API failure, empty document)

## Open Questions
- Should we store uploaded documents? (Privacy concern — currently we do NOT store them)
- What's the max document length we can reliably analyze? (Currently capped at 50,000 characters)
